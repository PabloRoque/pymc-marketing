...
class SpecialPrior(BaseModel, ABC):
    ...
    dim_handler: Any = (
        None  # Callable created dynamically, set during create_variable()
    )
...
class MaskedPrior(BaseModel):
    ...
    def serialize_prior(self, value: Prior) -> dict[str, Any]:
        ...
        """
        return value.to_dict() if hasattr(value, "to_dict") else None  # type: ignore[return-value]
    ...
    def _validate_mask(self) -> None:
        if tuple(self.mask.dims) != tuple(self.dims):  # type: ignore[arg-type]
    ...
    def _remap_dims(self, factory: VariableFactory) -> VariableFactory:
        ...
        if hasattr(factory, "dims"):
            ...
            if isinstance(dims, str):
                dims = (dims,)
            if tuple(dims) == tuple(self.dims):  # type: ignore[arg-type]
