...
class MMM(RegressionModelBuilder):
    ...
    def _serialize_mu_effect(effect: MuEffect) -> dict[str, Any]:
        ...
        if isinstance(effect, FourierEffect):
            ...
            dumped["fourier"] = effect.fourier.to_dict()  # type: ignore
            ...
            dumped["event_names"] = list(effect.df_events["name"].values)  # type: ignore
    ...
    def _model_config_formatting(cls, model_config: dict) -> dict:
        ...
        Special handling: Wrapped format objects (dicts with "class" key) are left
        untouched so they can be properly deserialized by custom deserializers.
        ...
        def format_nested_dict(d: dict) -> dict:
            # Check if this is a wrapped format object (e.g., MaskedPrior, special effects)
            # If so, don't convert lists to arrays as they need to be deserialized properly
            if "class" in d and "data" in d:
                # This is a wrapped format object - don't modify it
                return d
